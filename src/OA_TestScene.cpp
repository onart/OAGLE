...
namespace onart {
	pSafeAudioStream p;
	void TestScene::Update() { Entity::get<UI::Button>("button1")->move(vec4(cosf(tp), sinf(tp), 1, 1)); }
	void TestScene::init() {
		...
		entities.push_back(new UI::Button("button1", vec4(-1.0f / 1024, -1.0f / 1024, 0, 0), vec4(-0.5f, -0.5f, 1, 1), nullptr));
	}
}